...
/********************************************************/
template <typename T> void BStarTree<T>::rotateleft(Node *source, T v) {
...
// method
template <typename T> void BStarTree<T>::rotateRight(Node *source, T v) {
    // We store a reference to the parent's value that's to be used
    T &valueInParent =
        source->parent->values[source->parent->children.searchIndex(source)];
...
/********************************************************/
template <typename T> void BStarTree<T>::splitRight(Node *overloaded) {
    ...
    // We create a queue to save the children of the overloaded node and it's
    // sibling
    Queue<Node *> totalChildren;
    // We fill the queue with the overloaded node's children, as well as it's
    // sibling's
    for (int i = 0; i < overloaded->children.getSize(); ++i)
        totalChildren.enqueue(overloaded->children[i]);
    for (int i = 0; i < sibling->children.getSize(); ++i)
        totalChildren.enqueue(sibling->children[i]);
    ...
    // We check if the parent didn't become overloaded itself, and handle the
    // situation
...
/********************************************************/
template <typename T> void BStarTree<T>::splitLeft(Node *overloaded) {
    // Declaration of list of values
    OrderedList<T> totalValues;
    // We save relevant memory locations
...
BStarTree<T>::Node::Node(OrderedList<T> vals, DoubleLinkedList<Node *> s,
                         Node *p)
: values(vals), children(s), parent(p) {}
/********************************************************/
template <typename T>
BStarTree<T>::Node::Node(T v, DoubleLinkedList<Node *> s, Node *p)
: children(s), parent(p) {
    values.add(v);
}
/********************************************************/
template <typename T> BStarTree<T>::Node::Node(Node *p) : parent(p) {}

/********************************************************/
template <typename T> bool BStarTree<T>::Node::isRoot() const {
